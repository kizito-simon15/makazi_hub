...
def generate_houses_by_region_chart():
    ...
    region_counts = {}
    for house in houses:
        region = house.region.strip() if house.region else "Unknown"
    ...
    # Get the regions with max and min (if multiple, just pick one or list them)
    ...
    if max_region and min_region:
        # If there's one top region and one bottom region, we can craft advice
        # If multiple, just pick the first for simplicity
        top = max_region[0]
        bottom = min_region[0]
        advice = (
        ...
        # Generic advice if we can't determine max/min (e.g., only one region exists)
        advice = (
...
def generate_rooms_pie_chart():
    ...
    We'll base this on the number of 'Room' objects per region, or if needed, 
    we can count them directly from House.rooms related objects.
    ...
    Returns:
        ...
        rooms_per_region (dict): Mapping of {region_name: room_count}
    """
    ...
    # Compute percentages
    # Matplotlib's pie will handle percentages if we use 'autopct'
    ...
    # Generate advice
    # For example, if one region dominates, encourage balancing. If distribution is fairly even, suggest maintaining.
    max_count = max(counts)
    min_count = min(counts)
