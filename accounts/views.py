...
def owner_details_view(request):
    # Ensure the user is a property owner
    if not hasattr(request.user, 'property_owner'):
        messages.error(request, "Access denied: You are not a property owner.")
        return redirect('login')

    # Retrieve the property owner's details
    property_owner = request.user.property_owner

    context = {
        'username': request.user.username,
        'fullname': f"{property_owner.first_name} {property_owner.middle_name or ''} {property_owner.surname}".strip(),
        'email': request.user.email,
        'phone_number1': request.user.phone_number1,
        'phone_number2': request.user.phone_number2 or "N/A",
        'is_allowed': property_owner.is_allowed,
    }

    return render(request, 'accounts/owner_details.html', context)
